...
ただ、PPPoEではそのようなポート制限がないため、１ギガ回線ではIPv6だけIPoEで任せて、IPv4は引き続きPPPoE接続を利用するのも良いかと考えます。  
GoogleやYouTubeをはじめ大手サイトはIPv6対応のためIPv6でつながりますし、既存のIPv4サイトもそのうちIPv6へと移行しますので、これが正解な気もします。  
余談ですが、「IPoEだから高速」・「IPv6だから高速」といった安易なキャッチコピーは正しくありません。NTT回線以外はPPPoE接続が多いですがスピードもありますし、IPv4とIPv6は郵便番号でいうところの桁が違うぐらいの話です。コストのかかるIPv4のPPPoE接続をやめさせて、IPv4通信を集団でNTT IPoE IPv4 over IPv6の檻に閉じ込めるわけですから闇ですね。
