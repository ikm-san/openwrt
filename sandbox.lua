...
local function find_ipv4_prefix(ipv6_addr)
    -- IPv6アドレスから最初の32ビットを抽出する正規表現を修正
    local hex_prefix_part1, hex_prefix_part2 = ipv6_addr:match("^([a-fA-F0-9]{1,4}):([a-fA-F0-9]{1,4})")
    if not hex_prefix_part1 or not hex_prefix_part2 then
        return nil, "Invalid IPv6 address format"
    end

    -- 16進数のプレフィックスを結合して32ビット整数に変換
    local ipv6_prefix_32bit = tonumber(hex_prefix_part1 .. hex_prefix_part2, 16)
    ...
    for prefix, ipv4 in pairs(ipv6_prefix_map) do
        ...
        local prefix_32bit = tonumber(normalized_prefix, 16)
